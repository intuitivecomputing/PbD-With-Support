...
enum  Requests {TEACH=0, STOP_TEACH=1, ADD_WAYPOINT=2, DEL_WAYPOINT=3, MOVE_WAYPOINT=4, REP_WAYPOINT=5, EXEC=6, SELECT=7, EXEC_UNTIL=8, CLEAR_PROG=9};  // Types of requests from action client
...
void execute(const prog_support_backend::InterfaceRequestGoalConstPtr& goal, Server* as, ros::NodeHandle &node_handle)
{
  ...
  else if (goal->request == MOVE_WAYPOINT)
  {
    ...
    result.conclusion = "Swapped waypoint from index " + std::to_string(goal->optional_index_1) + " with " + std::to_string(goal->optional_index_2);
    as->setSucceeded(result);
    marker_package::Waypoint msg;
    msg.num = 0;
...
{
  ros::init(argc, argv, "baseline_interface");
